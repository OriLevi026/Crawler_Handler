...
#--------------------------------------------------------STEERING FUNCTIONS  1880 - 1060  
def SteerServo(steer):# SAME AS MOVE SERVO - DIFFRENT BOUNDS
    return STEER_SERVO.set_servo_pulsewidth(steerPin,steer)
    
    
def AxisToRPM(val):
    return (val + 1 )*500 +800

def AxisToPulseWidth (val):
    return float((val +1)*410 +1060)
...
pi = pigpio.pi()
pi.set_servo_pulsewidth(ESC_pin,0)
STEER_SERVO = pigpio.pi()
STEER_SERVO.set_servo_pulsewidth(steerPin,0)
time.sleep(0.1)
STEER_SERVO.set_servo_pulsewidth(steerPin,1360)
...
while done==False:
    ...
    for event in pygame.event.get(): # User did something
        ...
        if event.type == pygame.JOYAXISMOTION:# STEERING & THROTTLE FUNCTION----------------------
             if (event.axis == 0):
                 steer_axis = AxisToPulseWidth(joystick.get_axis(0)) # phisical bounds are 1060 to  1880
                 STEER_SERVO.set_servo_pulsewidth(steerPin,steer_axis)
